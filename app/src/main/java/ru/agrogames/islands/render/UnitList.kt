...
class UnitList(private val ui: UI, private val x: Float) {
    ...
    fun setUnits(units: Array<TransportUnit>) {
        ...
        for (unit in setOf(*units)) {
            ...
            elements.add(ui.createElement(
                Element(
                ElementType.Button, x, y, 1.2f, 1.2f, TextureMapper.join(
                    unit.example.texture,
                    if (unit == currentUnit) "selected" else "normal"
                ), renderInBorders = false)
            ))
            for (weapon in unit.example.weapons) {
                val e1 = ui.createElement(
                    Element(
